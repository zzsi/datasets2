...
def save_as_parquet(ds, outdir: str, split: str="train", num_shards=2, embed_external_files=True):
    os.makedirs(outdir, exist_ok=True)
    # Find decodable columns, because if there are any, we need to:
    # embed the bytes from the files in the shards
    decodable_columns = (
        [k for k, v in ds._info.features.items() if require_decoding(v, ignore_decode_attribute=True)]
    ...
    if num_shards is None:
        max_shard_size = convert_file_size_to_int(max_shard_size or datasets_config.MAX_SHARD_SIZE)
    ...
    #     print("Resuming upload of the dataset shards.")
